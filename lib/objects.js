...
class RGBRibbon extends Dimmer {
	constructor(namespace,name,address,desc = name) {
		super(namespace,name,address,desc);
		this.states.push(new State(namespace,'currentColor','number','Current color of RGB ribbon'));
	}
}
...
class Sensor extends Channel {
	constructor(namespace,name,address,desc = name) {
		super(namespace,name,address,desc);
		this.states.push(new State(namespace,'isLowBattery','boolean','state of sensor`s battery',false));
}

class RemoteControl extends Sensor {
	constructor(namespace,name,address,desc = name) {
		super(namespace,name,address,desc);
		this.states.push(new State(namespace,'command','string','last command from remote control on the wall',false));
	}
}
