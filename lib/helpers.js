...
class SimpleRelay extends Channel {
	constructor(namespace,name,address,desc) {
		...
		this.states.push(new State(namespace + '.' + name,'status','boolean','state of light switcher'));
	}
	getRole() {
		return 'light';
	}
...
class Dimmer extends SimpleRelay {
	constructor(namespace,name,address,desc,role = 'light.dimmer') {
		...
		this.states.push(new State(namespace + '.' + name,'brightness','number','Light brightness ',true,0,100));
	}
	getTypeCode() {
		return 3;
	}
...
class RGBRibbon extends Dimmer {
	constructor(namespace,name,address,desc) {
		...
		this.states.push(new State(namespace + '.' + name,'currentColor','string','Current color of RGB ribbon'));
	}
	getTypeCode() {
		return 4;
	}
...
class Sensor extends Channel {
	constructor(namespace,name,address,desc,role) {
		super(namespace,name,address,desc,role);
		this.states.push(new State(namespace + '.' + name,'onLowBattery','boolean','state of sensor`s battery',false));
	}
}
...
class RemoteControl extends Sensor {
	constructor(namespace,name,address,desc) {
		...
		this.states.push(new State(namespace + '.' + name,'onTurnOn','boolean','',false));
		this.states.push(new State(namespace + '.' + name,'onTurnOff','boolean','',false));
		this.states.push(new State(namespace + '.' + name,'onExecuteScenario','boolean','',false));
		this.states.push(new State(namespace + '.' + name,'onSaveScenario','boolean','',false));
		this.states.push(new State(namespace + '.' + name,'onStopReq','boolean','',false));
		this.states.push(new State(namespace + '.' + name,'onRollColour','boolean','',false));
		this.states.push(new State(namespace + '.' + name,'onSwitchColour','boolean','',false));
		this.states.push(new State(namespace + '.' + name,'onSwitch','string','boolean',false));
		this.states.push(new State(namespace + '.' + name,'onSensTempHumi','boolean','',false));
	}
	getTypeCode()  {
		return 0;
	}
...
class DoorSensor extends Sensor {
	constructor(namespace,name,address,desc) {
		...
		this.states.push(new State(namespace + '.' + name,'isOpen','boolean','The door sensor status',false));
	}
	getTypeCode() {
		return 1;
	}
...
class WaterSensor extends Sensor {
	constructor(namespace,name,address,desc) {
		...
		this.states.push(new State(namespace + '.' + name,'onAlarm','boolean','The water sensor status',false));
	}
	getTypeCode() {
		return 2;
	}
...
class MotionSensor extends Sensor {
	constructor(namespace,name,address,desc = name) {
		...
		this.states.push(new State(namespace + '.' + name,'MotionDetect','boolean','The motion sensor status',false));
	}
	getTypeCode() {
		return 6;
	}
