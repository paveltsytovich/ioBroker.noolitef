...
class Dimmer extends SimpleRelay {
	constructor(namespace,name,address,desc) {
		...
		this.states.push(new State(namespace + '.' + name,'startreg','number','Light dim ','level.dimmer',true,0,1));
		this.states.push(new State(namespace + '.' + name,'stopreg','number','Light dim ','level.dimmer',true));
	}
	getTypeCode() {
		return 3;
	}
