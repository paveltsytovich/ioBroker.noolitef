...
class RemoteControl extends Sensor {
	constructor(namespace,name,address,desc) {
		...
		this.states.push(new State(namespace + '.' + name,'onStopReq','boolean','Fire if long press switch button'));
		...
		this.states.push(new State(namespace + '.' + name,'onSwitch','string','Fire if press On/Off button'));
		this.states.push(new State(namespace + '.' + name,'onSensTempHumi','boolean','Fire if sensor change parameters'));
	}
	getTypeCode()  {
		return 0;
	}
