...
class KeysDevice extends MTRF64Driver.RemoteControl {
...
class DoorSensorDevice extends  MTRF64Driver.RemoteControl {
	constructor(controller,channel,mode,callback,name,mqtt = null) {
		super(controller,channel.mode);
		this.name = name;
		this.mqtt = mqtt;
	...
	onTurnOn() {
		this.callback(this.name, 'isOpen',true);
	}
	onTurnOff() {
		this.callback(this.name, 'isOpen',false);

	}
}

class WaterSensorDevice extends  MTRF64Driver.RemoteControl {
	constructor(controller,channel,mode,callback,name,mqtt = null) {
		super(controller,channel.mode);
		this.name = name;
		this.mqtt = mqtt;
...
class MotionSensorDevice extends  MTRF64Driver.RemoteControl {
	constructor(controller,channel,mode,callback,name,mqtt = null) {
		super(controller,channel.mode);
		this.name = name;
		this.mqtt = mqtt;
...
module.exports.KeysDevice = KeysDevice;
